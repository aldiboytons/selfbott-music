...
module.exports = {
    ...
    play: function(song) {
        ...
        if(!song){
            utils.log("Gaada lagunya di queue")
            serverQueue.voiceChannel.leave();
            return queue.delete("queue");
        }

        utils.log(`Nyetel lagu : ${song.title}`)
        ...
        dispatcher.on('finish', () => {
            if(serverQueue.songs[0]) utils.log(`Selesai puter lagu : ${serverQueue.songs[0].title}`);
            else utils.log(`Selesai puterin semua lagu ,ampe puyeng si anjing.`);
